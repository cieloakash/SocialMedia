...
const fnCreatePost = async (post,imageFile) => {
    ...
    const {uploadedError} = await supabase.storage.from("post-image").upload(filePath, imageFile);

    if (uploadedError) throw new Error(uploadedError.message);


  const {data:publicUrl} = supabase.storage.from("post-image").getPublicUrl(filePath)

  const { data, error } =await supabase.from("posts").insert({...post,imageurl:publicUrl });
...
const CreatePost = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-4">
      ...
      <button type="submit"  className="bg-purple-500 text-white px-4 py-2 rounded cursor-pointer">Create Post</button>
    </form>
  );
};
